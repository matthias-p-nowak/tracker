...
INSERT IGNORE INTO tracker_Accounted (`Activity`, `Day`, `YearWeek`, `WeekDay`)
SELECT h.Parent, a.`Day`, a.`YearWeek`, a.`WeekDay`
FROM tracker_Accounted a JOIN ActHier h ON a.Activity = h.Activity ;

UPDATE tracker_Accounted a JOIN (
SELECT 
h.`Parent`, a.`Day`, SUM(a.`DayAccount`) AS 'Total'
FROM ActHier h JOIN tracker_Accounted a ON h.Activity = a.Activity
